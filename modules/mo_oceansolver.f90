    ...
    DO IX = 1,NX+2
       ...
       IF( (GrdInfo.PLon(IX) >= Nudging_WLon ).AND. ( GrdInfo.PLon(IX) <= Nudging_ELon ) )THEN
          IF( (GrdInfo.PLat(IY) >= Nudging_SLat ).AND. ( GrdInfo.PLat(IY) <= Nudging_NLat ) )THEN
           ...
           !( 1.0d0 + max(0.0d0, 1.0d0 - abs(GrdInfo.PLat(IY))/SSTA_Nudging_Radius ) ) * AllFrc_OcnStat.NUDGE(IX,IY) 
...
!!! get monthly mean stat
    ...
    IF( OutPut_HeatBDG_OnOff == 1 ) THEN
        ...
        !$omp section
        call Update_MonMe_Var (AllFrc_OcnStat.UATBX, AllFrc_OcnStat.MonMe_UATBX, &
             Arrive_CMonth_FLAG, Reach_CMonth_FLAG, Leap_Calendar_OnOff, CYear, CMonth, Day_NT)
        
        !$omp section
        call Update_MonMe_Var (AllFrc_OcnStat.UBTAX, AllFrc_OcnStat.MonMe_UBTAX, &
             Arrive_CMonth_FLAG, Reach_CMonth_FLAG, Leap_Calendar_OnOff, CYear, CMonth, Day_NT)
        
        !$omp section
        call Update_MonMe_Var (AllFrc_OcnStat.UATAX, AllFrc_OcnStat.MonMe_UATAX, &
             Arrive_CMonth_FLAG, Reach_CMonth_FLAG, Leap_Calendar_OnOff, CYear, CMonth, Day_NT)
        
        !$omp section
        call Update_MonMe_Var (AllFrc_OcnStat.VATBY, AllFrc_OcnStat.MonMe_VATBY, &
             Arrive_CMonth_FLAG, Reach_CMonth_FLAG, Leap_Calendar_OnOff, CYear, CMonth, Day_NT)
        
        !$omp section
        call Update_MonMe_Var (AllFrc_OcnStat.VBTAY, AllFrc_OcnStat.MonMe_VBTAY, &
             Arrive_CMonth_FLAG, Reach_CMonth_FLAG, Leap_Calendar_OnOff, CYear, CMonth, Day_NT)
        
        !$omp section
        call Update_MonMe_Var (AllFrc_OcnStat.VATAY, AllFrc_OcnStat.MonMe_VATAY, &
             Arrive_CMonth_FLAG, Reach_CMonth_FLAG, Leap_Calendar_OnOff, CYear, CMonth, Day_NT)
        
        !$omp section
        call Update_MonMe_Var (AllFrc_OcnStat.WATBZ, AllFrc_OcnStat.MonMe_WATBZ, &
             Arrive_CMonth_FLAG, Reach_CMonth_FLAG, Leap_Calendar_OnOff, CYear, CMonth, Day_NT)
        
        !$omp section
        call Update_MonMe_Var (AllFrc_OcnStat.WBTAZ, AllFrc_OcnStat.MonMe_WBTAZ, &
             Arrive_CMonth_FLAG, Reach_CMonth_FLAG, Leap_Calendar_OnOff, CYear, CMonth, Day_NT)
        
        !$omp section
        call Update_MonMe_Var (AllFrc_OcnStat.WATAZ, AllFrc_OcnStat.MonMe_WATAZ, &
             Arrive_CMonth_FLAG, Reach_CMonth_FLAG, Leap_Calendar_OnOff, CYear, CMonth, Day_NT)
        
        !$omp section
        call Update_MonMe_Var (AllFrc_OcnStat.SSTA_RESQ, AllFrc_OcnStat.MonMe_SSTA_RESQ, &
             Arrive_CMonth_FLAG, Reach_CMonth_FLAG, Leap_Calendar_OnOff, CYear, CMonth, Day_NT)
