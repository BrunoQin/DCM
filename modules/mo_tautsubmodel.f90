...
SUBROUTINE PostProc_TAUXY (GrdInfo, TimTick, TAUX_TAUY_PGrid, OcnStat, Clim_OcnStat, SrfFlux, Test_OnOff)
    ...
    END DO
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !!! record internnal wind stress
    SrfFlux.IntAn_TAUX = SrfFlux.TAUX
    SrfFlux.IntAn_TAUY = SrfFlux.TAUY
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    END IF !!! Test_OnOff
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!! Regression Model onto Nino4 & Nino3 Index
    ...
    real*8,allocatable :: TAUX_TAUY_PGrid(:,:,:)
    real*8,allocatable :: Wind_Nudging_Ratio_Mask(:,:,:) !!! space-dependent wind nudging ratio 
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    IF ( mod (TimTick.Run_AbsIT, (Day_NT)*Tau_Couple_N_Day) == 1 )THEN
        ...
        allocate(   TAUX_TAUY_PGrid (AI_PNX, AI_PNY, 2));
        allocate(Wind_Nudging_Ratio_Mask(AI_PNX,AI_PNY,2));
        
        Ext_TAUX_TAUY_PGrid = 0.0d0;
        Cpl_TAUX_TAUY_PGrid = 0.0d0;
        TAUX_TAUY_PGrid = 0.0d0;
        Wind_Nudging_Ratio_Mask = 1.0d0;
        
        !!! Set Wind Nudging Ratio Mask
        !DO IY = 1,AI_PNY
        !   Wind_Nudging_Ratio_Mask(1:AI_PNX,IY,1:2) = &
        !   max(0.0d0, 1.0d0 - abs(GrdInfo.PLat(IY))/(Wind_Nudging_Radius) );
        !END DO
        ...
        !!! Wind Nudging Ratio can also vary with Space
        TAUX_TAUY_PGrid = (Wind_Nudging_Ratio * Wind_Nudging_Ratio_Mask) * Ext_TAUX_TAUY_PGrid + &
                          (1.0d0 - Wind_Nudging_Ratio * Wind_Nudging_Ratio_Mask) * Cpl_TAUX_TAUY_PGrid
        ...
        !!! deallocate TAUX_TAUY_PGrid
        deallocate(Ext_TAUX_TAUY_PGrid, Cpl_TAUX_TAUY_PGrid)
        deallocate(TAUX_TAUY_PGrid)
        deallocate(Wind_Nudging_Ratio_Mask);
...
SUBROUTINE Recon_MJO_Forcing_PsTauXY( GrdInfo, TimTick, OcnStat, Clim_OcnStat, SrfFlux, YearDay)
    ...
    DO IEEOF = 1,SrfFlux.NEEOF
        ...
        SrfFlux.CFM_MJO_PsTauX = SrfFlux.CFM_MJO_PsTauX + ( SrfFlux.PC_MJO_PsTauXY(IEEOF) ) * &
                                 ( Clim_OcnStat.CFM_EEOF_PsTauX(1:CFM_NOcn,1:NY+2,YearDay,IEEOF) );
    ...
    !!!! gain positive value
    SrfFlux.MJO_PsTauX = (1.0d0)*(0.5d0)*(SrfFlux.MJO_PsTauX + abs(SrfFlux.MJO_PsTauX)) + &
    (Add_MJO_EastWind_Ratio)*(0.5d0)*(SrfFlux.MJO_PsTauX - abs(SrfFlux.MJO_PsTauX))
    
    
    SrfFlux.CFM_MJO_PsTauX = (1.0d0)*(0.5d0)*(SrfFlux.CFM_MJO_PsTauX + abs(SrfFlux.CFM_MJO_PsTauX)) + &
    (Add_MJO_EastWind_Ratio)*(0.5d0)*(SrfFlux.CFM_MJO_PsTauX - abs(SrfFlux.CFM_MJO_PsTauX))

    !!! Filter WWB Threshold
    IF( Filter_WWB_OnOff == 1) THEN

    DO IX = 1,NX+1
       DO IY = 1,NY
          
          IF( SrfFlux.MJO_PsTauX(IX,IY) < ( WWB_Threshold/(CoeffDrag * RhoAtm) ) )THEN

              SrfFlux.MJO_PsTauX(IX,IY) = 0.0d0;

          ENDIF

          IF( SrfFlux.MJO_PsTauX(IX,IY) > ( WWB_Upperbound/(CoeffDrag * RhoAtm) ) )THEN

              SrfFlux.MJO_PsTauX(IX,IY) = ( WWB_Upperbound/(CoeffDrag * RhoAtm) )
    ...
    DO IX = 1,CFM_NOcn
       DO IY = 1,NY+2
          ...
          IF( SrfFlux.CFM_MJO_PsTauX(IX,IY) < ( WWB_Threshold/(CoeffDrag * RhoAtm) ) )THEN

              SrfFlux.CFM_MJO_PsTauX(IX,IY) = 0.0d0;

          ENDIF

          IF( SrfFlux.CFM_MJO_PsTauX(IX,IY) > ( WWB_Upperbound/(CoeffDrag * RhoAtm) ) )THEN

              SrfFlux.CFM_MJO_PsTauX(IX,IY) = ( WWB_Upperbound/(CoeffDrag * RhoAtm) )
    ...
    ENDIF !!! Filter_WWB_OnOff

    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    
    !!!! project 2D to 1D Kelvin Mode
    DO IX = 1, CFM_NOcn
    
       SrfFlux.MJO_KelCoeff(IX) = &
       sum( Clim_OcnStat.CFM_KelMode(IX,1:NY+2) * SrfFlux.CFM_MJO_PsTauX(IX,1:NY+2) )/dble(NY+2);
...
SUBROUTINE Add_MJO_Forcing_TAUXY ( GrdInfo, TimTick, OcnStat, Clim_OcnStat, CFM_Stat, SrfFlux)
    ...
    IF (TimTick.Run_Cycle_ID == 1) THEN
        ...
        ! SrfFlux.PC_MJO_PsTauXY = SrfFlux.PC_MJO_PsTauXY_Guess;
    
    ELSEIF( TimTick.Run_Cycle_ID >= 2) THEN
    
        !!! Historical Perturbation Test
        ! SrfFlux.PC_MJO_PsTauXY = Clim_OcnStat.PC_PsTauXY(1:NEEOF,TimTick.Run_Cycle_ID-1);

        SrfFlux.PC_MJO_PsTauXY = SrfFlux.PC_MJO_PsTauXY_Guess;
       
        !!! Continuity Test (1.0d-0 ~ 1.0d-15)
        !!! SrfFlux.PC_MJO_PsTauXY = (10.0d0**((-1.0d0)*dble(TimTick.Run_Cycle_ID-2))) * SrfFlux.PC_MJO_PsTauXY_Guess
        
        !!! Grad Compuation
        !SrfFlux.PC_MJO_PsTauXY = SrfFlux.PC_MJO_PsTauXY_Guess;
        
        !SrfFlux.PC_MJO_PsTauXY(TimTick.Run_Cycle_ID + Grad_EEOF_StartID-1) = &
        !SrfFlux.PC_MJO_PsTauXY(TimTick.Run_Cycle_ID + Grad_EEOF_StartID-1) + (Grad_EEOF_Step);

        !!! Grad Validation
        !SrfFlux.PC_MJO_PsTauXY = SrfFlux.PC_MJO_PsTauXY_Guess
        !SrfFlux.PC_MJO_PsTauXY = SrfFlux.PC_MJO_PsTauXY_Guess + &
        !                        (10.0d0**((-1.0d0)*dble(TimTick.Run_Cycle_ID-2))) * SrfFlux.PC_MJO_PsTauXY_Guess_Grad

        
        !!! CNOP Validation
        IF( (Valid_Mode==1).AND.(TimTick.Run_Cycle_ID>= 3) ) THEN

            SrfFlux.PC_MJO_PsTauXY = SrfFlux.PC_MJO_PsTauXY_Guess;

            SrfFlux.PC_MJO_PsTauXY = SrfFlux.PC_MJO_PsTauXY + &
                                     (Valid_Radius)*Clim_OcnStat.PC_PsTauXY(1:NEEOF,TimTick.Run_Cycle_ID-2);

            !!! norm constraint
            IF( sqrt(sum(SrfFlux.PC_MJO_PsTauXY*SrfFlux.PC_MJO_PsTauXY)) > CNOP_Radius ) THEN

                SrfFlux.PC_MJO_PsTauXY = &
                ( CNOP_Radius/sqrt(sum(SrfFlux.PC_MJO_PsTauXY*SrfFlux.PC_MJO_PsTauXY)) ) * SrfFlux.PC_MJO_PsTauXY;
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    IF( (TimTick.Run_AbsIT - TimTick.Res_AbsIT) > (Day_NT)*(365+31+28) ) THEN
         SrfFlux.PC_MJO_PsTauXY = 0.0d0;
    END IF
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    !!!! Add Forcing at different Time
  
    !IF( TimTick.Run_Cycle_ID >= 3) THEN
    !   IF( (TimTick.Run_AbsIT - TimTick.Res_AbsIT) > (Day_NT)*(90*(TimTick.Run_Cycle_ID-2)) )THEN
    !       SrfFlux.PC_MJO_PsTauXY = 0.0d0;
    !   END IF
    !END IF
    ...
    IF ( mod (TimTick.Run_AbsIT, Day_NT) == 1 )THEN
         ...
         !!! Only Add Forcing from  Jan (0) to Feb (1)
         IF( (TimTick.Run_AbsIT - TimTick.Res_AbsIT) > (Day_NT)*(Add_MJO_Stop_Day)) THEN 
             SrfFlux.MJO_PsTauX   = 0.0d0;
             SrfFlux.MJO_PsTauY   = 0.0d0;
             SrfFlux.MJO_KelCoeff = 0.0d0;
         END IF
         ...
         IF(Add_ENBOM_MJO_Forcing_OnOff == 1 )THEN
            SrfFlux.TAUX = SrfFlux.IntAn_TAUX + CoeffDrag * RhoAtm * SrfFlux.MJO_PsTauX 
            SrfFlux.TAUY = SrfFlux.IntAn_TAUY + CoeffDrag * RhoAtm * SrfFlux.MJO_PsTauY
         ENDIF
