...
character(len = 100), parameter :: Model_Path     = '/mnt/sda/JYW_AtmOcn_Model/ELBOM/Experiments/exp-dualcore/'
character(len = 100), parameter :: InputData_Path = '/mnt/sda/JYW_AtmOcn_Model/ELBOM/Experiments/exp-dualcore/'
...
character(len=100),parameter :: Grid_Path    = trim(InputData_Path)//'input/';
character(len=100),parameter :: ClimOcn_Path = trim(InputData_Path)//'input/Model_Clim_OceanStat/';
character(len=100),parameter :: Forcing_Path = trim(InputData_Path)//'input/Model_Forcing/';
...
!!! test TSUB-AI
character(len=100),parameter :: SSH_Test_FName='MonAn_Mask_SSH_GODAS_1981_2023.nc';
...
!!! Gradient associated with CNOP First Guess Txt File
character(len=100),parameter :: MJO_Forcing_PC_Guess_Grad_FName='PC_PsTauXY_Guess_Grad.txt'
...
!!! OpenMP Speed-UP
integer*4,parameter :: MY_OMP_THREADS = 16
...
!!! end at 24:00:00
integer*8,parameter :: End_CYear   = 100, End_CMonth = 12, End_CDay = 31
...
integer*8,parameter :: ReStart_N_Unit = 1001
...
integer*8,parameter :: OutPut_Day_Span = 424

!!! how many years are there for one output file
integer*8,parameter :: OutPut_Year_Span = 100

!!!! putdata into OutPut file every (PutData_N_Day) day
!!!!               and just at the end integration step of a new round (N_Day)
integer*8,parameter :: PutData_N_Day   = 1
...
integer*8,parameter :: MonMe_OutPut_Mon_Span = 12*OutPut_Year_Span
...
integer*8 :: Add_MJO_Forcing_OnOff = 1;       !!! Add MJO_Forcing_OnOff

real*8    :: Add_MJO_EastWind_Ratio = 1.0d0;  !!! Add MJO EastWind Ratio (0.0d0~1.0d0) 
...
integer*8 :: Add_MJO_Stop_Day = 424;          !!! When MJO_Forcing Stop (unit:Day)

integer*8 :: Filter_WWB_OnOff = 0;             !!! Filter WWB of given threshold (0.05 N/m^2)
real*8,parameter :: WWB_Threshold  = 0.05d0    !!! WWB threshold  (N/m^2)
real*8,parameter :: WWB_Upperbound = 0.25d0    !!! WWB upperbound (N/m^2)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

integer*8 :: Eq_Direct_Assign   = 0;     !!! direct assign value when nudging

real*8 :: SSTA_Nudging_Scale = 60.0d0    !!! unit: day (60.0d0)
real*8 :: Wind_Nudging_Ratio = 1.0d0     !!! the ratio of external wind (1.0d0)

real*8 :: SSTA_Nudging_Radius = 5.0d0   !!! unit: degree
real*8 :: Wind_Nudging_Radius = 15.0d0  !!! unit: degree
...
!!! Compute Gradient Info for CNOP
integer*8,parameter  :: Valid_Mode = 0  !!! Valide Mode (1) or not (0) (==1: no 2D output)
real*8,parameter     :: CNOP_Radius  = 6.4d0 !!! constrain radius,6.4d0 for MJO,NLFV, 12.7d0 for WWB
real*8,parameter     :: Valid_Radius = 1.0d0 !!! (0.0d0 ~ 1.0d0)

integer*8,parameter  :: Grad_Mode = 0 !!! F Operator Mode (Grad_Mode=0) or G Operator Mode (1) 
integer*8,parameter  :: Grad_Fork_ID = 1
integer*8,parameter  :: Grad_EEOF_StartID = 5*(Grad_Fork_ID-1)+1
real*8,parameter     :: Grad_EEOF_Step    = 1.0d-1  
...
!!! in MAIN PROGRAM  
integer*8,parameter :: Run_Cycles    = 2+40*Valid_Mode;   !!! maximum cycles of running the main program (not main cycle) 
integer*8,parameter :: Run_OnOff     = 1;   !!! run main cycle (1) or not (0)
integer*8,parameter :: ReRun_OnOff   = 1;   !!! restart run (1) or not (0)
...
!!! INSTANT OUTPUT 
integer*8,parameter :: Inst_OutPut_OnOff = 1; 

!!! MONTHLY MEAN OUTPUT (FOR LONG-TERM SIMULATION TO SAVE SPACE)
integer*8,parameter :: MonMe_OutPut_OnOff = 0; 


!!! Clip_Run on (1) or not (0, Calendar Run)
integer*8,parameter :: Clip_Run_OnOff = 1
integer*8,parameter :: Clip_Run_NDay  = 424

integer*8,parameter :: CFModel_RunAlone_OnOff = 0;   !!! CF23 Model Run Alone (1) or not (0)

integer*8,parameter :: ENBOM_OutPut_2D_OnOff   = 0*(1-Valid_Mode); !!! ENBOM Model OutPut 2D Field (1) or not (0) to save space
integer*8,parameter :: CFModel_OutPut_2D_OnOff = 0*(1-Valid_Mode); !!! CF23 Model OutPut 2D Field (1) or not (0) to save space
...
integer*8,parameter :: OutPut_HeatBDG_OnOff  = 0*(1-Valid_Mode)
...
real*8,parameter :: TSUB_OutPut_Amp  = 0.9d0    !!! 0.9d0 TSUB output amplitude factor
...
TYPE Clim_OceanStat
     ...
     real*8,allocatable :: EEOF_PsTauX(:,:,:,:) !!! (NX+2,NY+2,365+31+28,NEEOF)
     real*8,allocatable :: EEOF_PsTauY(:,:,:,:) !!! (NX+2,NY+2,365+31+28,NEEOF)
     
     real*8,allocatable :: CFM_EEOF_PsTauX(:,:,:,:) !!! (CFM_NOcn,NY+2,365+31+28,NEEOF)
     
     real*8,allocatable :: PC_PsTauXY(:,:)      !!! (NEEOF,NYear)
     real*8,allocatable :: EEOF_KelCoeff(:,:,:) !!! (CFM_NOcn,365,NEEOF)
     real*8,allocatable :: CFM_KelMode(:,:)     !!! (CFM_NOcn,NY+2)
...
TYPE AllFrc_OceanStat !!! right-hand forcing terms F(U)
    ...
    !!! monthly mean state of anomalies for simple heat budget analysis
    real*8,allocatable :: MonMe_ZAFK(:,:),MonMe_THFK(:,:),MonMe_RESFK(:,:),MonMe_NUDGE(:,:)
    
    !!! monthly mean state of anomalies for full heat budget analysis
    
    real*8,allocatable :: MonMe_UATBX(:,:), MonMe_UBTAX(:,:), MonMe_UATAX(:,:)
    real*8,allocatable :: MonMe_VATBY(:,:), MonMe_VBTAY(:,:), MonMe_VATAY(:,:)
    real*8,allocatable :: MonMe_WATBZ(:,:), MonMe_WBTAZ(:,:), MonMe_WATAZ(:,:)
    real*8,allocatable :: MonMe_SSTA_RESQ(:,:) 
...
TYPE SurfaceFlux !!! sea surface flux into the ocean 
    ...
    !!! interannual momentum flux
    real*8,allocatable :: IntAn_TAUX(:,:) !!! (NX+1,NY)
    real*8,allocatable :: IntAn_TAUY(:,:) !!! (NX,NY+1)
    ...
    real*8,allocatable :: CFM_MJO_PsTauX(:,:) !!! (CFM_NOcn,NY+2),be careful
    
    real*8,allocatable :: PC_MJO_PsTauXY(:) !!! (NEEOF)
    real*8,allocatable :: MJO_KelCoeff(:)   !!! (CFM_NOcn)
    
    !!! CNOP First Guess
    real*8,allocatable :: PC_MJO_PsTauXY_Guess(:) !!! (NEEOF)
    
    !!! Grad assoicated with CNOP First Guess (for Grad Test)
    real*8,allocatable :: PC_MJO_PsTauXY_Guess_Grad(:) !!! (NEEOF)
...
TYPE CFModel_Stat
    ...
    real*8 :: MonMe_IntDec, MonMe_ItrAmp
