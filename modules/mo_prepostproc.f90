...
SUBROUTINE Alloc_AllFrc_OceanStat (GrdInfo, AllFrc_OcnStat)
    ...
    !!! FULL HeatBDG
    allocate( AllFrc_OcnStat.MonMe_UATBX(NX+2,NY+2) );  AllFrc_OcnStat.MonMe_UATBX = 0.0d0;
    allocate( AllFrc_OcnStat.MonMe_UBTAX(NX+2,NY+2) );  AllFrc_OcnStat.MonMe_UBTAX = 0.0d0;
    allocate( AllFrc_OcnStat.MonMe_UATAX(NX+2,NY+2) );  AllFrc_OcnStat.MonMe_UATAX = 0.0d0;
    
    allocate( AllFrc_OcnStat.MonMe_VATBY(NX+2,NY+2) );  AllFrc_OcnStat.MonMe_VATBY = 0.0d0;
    allocate( AllFrc_OcnStat.MonMe_VBTAY(NX+2,NY+2) );  AllFrc_OcnStat.MonMe_VBTAY = 0.0d0;
    allocate( AllFrc_OcnStat.MonMe_VATAY(NX+2,NY+2) );  AllFrc_OcnStat.MonMe_VATAY = 0.0d0; 
    
    allocate( AllFrc_OcnStat.MonMe_WATBZ(NX+2,NY+2) );  AllFrc_OcnStat.MonMe_WATBZ = 0.0d0;
    allocate( AllFrc_OcnStat.MonMe_WBTAZ(NX+2,NY+2) );  AllFrc_OcnStat.MonMe_WBTAZ = 0.0d0;
    allocate( AllFrc_OcnStat.MonMe_WATAZ(NX+2,NY+2) );  AllFrc_OcnStat.MonMe_WATAZ = 0.0d0;
    
    allocate( AllFrc_OcnStat.MonMe_SSTA_RESQ(NX+2,NY+2) ); AllFrc_OcnStat.MonMe_SSTA_RESQ = 0.0d0;
...
SUBROUTINE  Alloc_SurfaceFlux(GrdInfo, SrfFlux)
    ...
    !!! interannual taux stress
    allocate( SrfFlux.IntAn_TAUX(NX+1, NY) ); 
    SrfFlux.IntAn_TAUX = 0.0d0;
    allocate( SrfFlux.IntAn_TAUY(NX, NY+1) ); 
    SrfFlux.IntAn_TAUY = 0.0d0;
    ...
    allocate( SrfFlux.CFM_MJO_PsTauX(SrfFlux.CFM_NOcn,NY+2) ); SrfFlux.CFM_MJO_PsTauX = 0.0d0;
    ...
    allocate( SrfFlux.PC_MJO_PsTauXY_Guess(SrfFlux.NEEOF) ); 
    SrfFlux.PC_MJO_PsTauXY_Guess = 0.0d0;
    
    allocate( SrfFlux.PC_MJO_PsTauXY_Guess_Grad(SrfFlux.NEEOF) );
    SrfFlux.PC_MJO_PsTauXY_Guess_Grad = 0.0d0;
    
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    
    !!! read First Guess Txt File as default value 
    
    !!! write(*,*),trim(trim(Forcing_Path)//trim(MJO_Forcing_PC_Guess_FName))
    
    open(unit=10, file=trim(trim(Forcing_Path)//trim(MJO_Forcing_PC_Guess_FName)), &
    ...
    num_lines = 0
    do i = 1, SrfFlux.NEEOF
        read(10, *, iostat=ios) SrfFlux.PC_MJO_PsTauXY_Guess(i)
        if (ios /= 0) exit  
    ...
    !!! OutPut Test
    print *, "First Guess File read successfully."
    !!! do i = 1, num_lines
    !!!    print *, SrfFlux.PC_MJO_PsTauXY(i)
    !!! end do
    
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    
    !!! read Grad assoicated with CNOP First Guess txt file
    open(unit=10, file=trim(trim(Forcing_Path)//trim(MJO_Forcing_PC_Guess_Grad_FName)), &
         status='old', action='read', iostat=ios)
    ...
    num_lines = 0
    do i = 1, SrfFlux.NEEOF
        read(10, *, iostat=ios) SrfFlux.PC_MJO_PsTauXY_Guess_Grad(i)
        if (ios /= 0) exit  
        num_lines = num_lines + 1
    end do
    ...
    !!! OutPut Test
    print *, "First Guess Grad File read successfully."
    !do i = 1, num_lines
    !    print *, SrfFlux.PC_MJO_PsTauXY_Guess_Grad(i)
    !end do
...
SUBROUTINE Read_Grid_ClimStat (GrdPath, ClimPath,FrcPath, GrdInfo, Clim_OcnStat)
    ...
    call NC_Read_Mul_IT &
         (FrcPath,MJO_Forcing_EEOF_FName,'CFM_EEOF_PsTauX',Clim_OcnStat.CFM_EEOF_PsTauX,0,0,'A');
    ...
    call NC_Read_Mul_IT &
         (FrcPath,MJO_Forcing_EEOF_FName,'CFM_KelMode',Clim_OcnStat.CFM_KelMode,0,0,'A');
    ...
    write(*,*),'' 
    write(*,*),'Forcing EEOF Reading completed'
    write(*,*),'-----------------------------------------------------------------------------'
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
SUBROUTINE Read_ReStart(ResPath,GrdInfo,OcnStat,TimTick,SrfFlux);
     ...
     write(6,124) TimTick.Run_Cycle_ID
     124  format(' TimTick.Run_Cycle_ID = ',I8)
...
SUBROUTINE Create_OutPut(OutPath,GrdInfo,OcnStat,TimTick,AllFrc_OcnStat,SrfFlux);
     ...
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     IF ( ENBOM_OutPut_2D_OnOff == 1 ) THEN
     ...
     ELSE
         ...
         call NC_Def_Var(OutPath,U_OutName,'IntAn_TAUX','N/m^2','lon','lat','time')
         call NC_Def_Var(OutPath,V_OutName,'IntAn_TAUY','N/m^2','lon','lat','time')
     ...
     END IF  !!! ENBOM_OutPut_2D_OnOff 
     
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     
     
     !!! Time series
     call NC_Def_Var(OutPath,P_OutName,'SSHA_N4','m','time',  DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,P_OutName,'SSHA_N34','m','time', DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,P_OutName,'SSHA_N3','m','time',  DOUBLE_FLAG=1)
     
     call NC_Def_Var(OutPath,P_OutName,'TSUB_N4','dC','time', DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,P_OutName,'TSUB_N34','dC','time',DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,P_OutName,'TSUB_N3','dC','time', DOUBLE_FLAG=1)
     
     call NC_Def_Var(OutPath,P_OutName,'SSTA_N4','dC','time', DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,P_OutName,'SSTA_N34','dC','time',DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,P_OutName,'SSTA_N3','dC','time', DOUBLE_FLAG=1)
     
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     IF(Clip_Run_OnOff == 0)THEN
     
     call NC_Def_Var(OutPath,U_OutName,'UA_N4','m/s','time', DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,U_OutName,'UA_N34','m/s','time',DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,U_OutName,'UA_N3','m/s','time', DOUBLE_FLAG=1)
     
     call NC_Def_Var(OutPath,U_OutName,'TAUX_N4','N/m^2','time', DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,U_OutName,'TAUX_N34','N/m^2','time',DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,U_OutName,'TAUX_N3','N/m^2','time', DOUBLE_FLAG=1)
     
     call NC_Def_Var(OutPath,U_OutName,'TAUX_N4_Test','N/m^2','time',  DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,U_OutName,'TAUX_N34_Test','N/m^2','time', DOUBLE_FLAG=1)
     call NC_Def_Var(OutPath,U_OutName,'TAUX_N3_Test','N/m^2','time',  DOUBLE_FLAG=1)
...
SUBROUTINE Write_OutPut(OutPath,GrdInfo,OcnStat,TimTick,AllFrc_OcnStat,SrfFlux,WriteIT);
     ...
     ELSE 
         ...
         call NC_Write_One_IT(OutPath,U_OutName,'TAUX',SrfFlux.TAUX,WriteIT);
         call NC_Write_One_IT(OutPath,U_OutName,'TAUX_Test',SrfFlux.TAUX_Test,WriteIT);
         call NC_Write_One_IT(OutPath,U_OutName,'IntAn_TAUX',SrfFlux.IntAn_TAUX,WriteIT);
     
         call NC_Write_One_IT(OutPath,V_OutName,'TAUY',SrfFlux.TAUY,WriteIT);
         call NC_Write_One_IT(OutPath,V_OutName,'TAUY_Test',SrfFlux.TAUY_Test,WriteIT);
         call NC_Write_One_IT(OutPath,V_OutName,'IntAn_TAUY',SrfFlux.IntAn_TAUY,WriteIT);
     ...
     END IF !!! ENBOM_OutPut_2D_OnOff == 0 
     ...
     IF (Solve_SSTA_OnOff == 1 ) THEN
        ...
        !!! Heat budget
         IF( OutPut_HeatBDG_OnOff == 1 ) THEN
             ...
             call NC_Write_One_IT(OutPath,P_OutName,'UATBX',AllFrc_OcnStat.MonMe_UATBX,WriteIT);
             call NC_Write_One_IT(OutPath,P_OutName,'UBTAX',AllFrc_OcnStat.MonMe_UBTAX,WriteIT);
             call NC_Write_One_IT(OutPath,P_OutName,'UATAX',AllFrc_OcnStat.MonMe_UATAX,WriteIT);
     
             call NC_Write_One_IT(OutPath,P_OutName,'VATBY',AllFrc_OcnStat.MonMe_VATBY,WriteIT);
             call NC_Write_One_IT(OutPath,P_OutName,'VBTAY',AllFrc_OcnStat.MonMe_VBTAY,WriteIT);
             call NC_Write_One_IT(OutPath,P_OutName,'VATAY',AllFrc_OcnStat.MonMe_VATAY,WriteIT);
     
             call NC_Write_One_IT(OutPath,P_OutName,'WATBZ',AllFrc_OcnStat.MonMe_WATBZ,WriteIT);
             call NC_Write_One_IT(OutPath,P_OutName,'WBTAZ',AllFrc_OcnStat.MonMe_WBTAZ,WriteIT);
             call NC_Write_One_IT(OutPath,P_OutName,'WATAZ',AllFrc_OcnStat.MonMe_WATAZ,WriteIT);
     
             call NC_Write_One_IT(OutPath,P_OutName,'SSTA_RESQ',AllFrc_OcnStat.MonMe_SSTA_RESQ,WriteIT);
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SUBROUTINE OutPut_MulRun_ObjFunction &
           (OutPath,GrdInfo,OcnStat,TimTick,AllFrc_OcnStat,SrfFlux,CFM_Par,CFM_Stat)

!!! multiple run output for CNOP objective function
     ...
     real*8,allocatable :: out_time_tick(:) !!! tmp timetick size == OutPut_N_Event
     character(len=100) :: time_unit   !!! time_unit
     character(len=300) :: ObjFunc_OutName
     
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     
     allocate(out_time_tick(Run_Cycles)) !!! MonMe_OutPut_N_Event == MonMe_OutPut_N_Span
     ...
     !!! time_unit
     time_unit = 'Run_Cycle_ID';
     
     ObjFunc_OutName = 'ObjFunc_'//trim(Math_YMD_To_Date(TimTick.CYear,TimTick.CMonth,TimTick.CDay))//'.nc'
     
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     IF (TimTick.Run_Cycle_ID == 1) THEN
     
         WriteIT = int4(TimTick.Run_Cycle_ID);
     
         write(*,*),'----------------------------------------------------------------------------'
         write(*,*),'Creating Multiple Run Objective Function File ...'

         call NC_Create_File(OutPath,ObjFunc_OutName,out_time_tick,'time',time_unit,Leap_Calendar_OnOff)
     
         call NC_Def_Var(OutPath,ObjFunc_OutName,'SSTA_N4','dC','time', DOUBLE_FLAG=1)
         call NC_Def_Var(OutPath,ObjFunc_OutName,'SSTA_N34','dC','time',DOUBLE_FLAG=1)
         call NC_Def_Var(OutPath,ObjFunc_OutName,'SSTA_N3','dC','time', DOUBLE_FLAG=1)
     
         call NC_Def_Var(OutPath,ObjFunc_OutName,'Eq_T4','dC','time', DOUBLE_FLAG=1)
         call NC_Def_Var(OutPath,ObjFunc_OutName,'Eq_T34','dC','time',DOUBLE_FLAG=1)
         call NC_Def_Var(OutPath,ObjFunc_OutName,'Eq_T3','dC','time', DOUBLE_FLAG=1)
     
         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     
         call NC_Write_One_IT(OutPath,ObjFunc_OutName,'SSTA_N4', OcnStat.SSTA_N4, WriteIT);
         call NC_Write_One_IT(OutPath,ObjFunc_OutName,'SSTA_N34',OcnStat.SSTA_N34,WriteIT);
         call NC_Write_One_IT(OutPath,ObjFunc_OutName,'SSTA_N3', OcnStat.SSTA_N3, WriteIT);
     
         call NC_Write_One_IT(OutPath,ObjFunc_OutName,'Eq_T4', CFM_Stat.Eq_T4,  WriteIT);
         call NC_Write_One_IT(OutPath,ObjFunc_OutName,'Eq_T34',CFM_Stat.Eq_T34, WriteIT);
         call NC_Write_One_IT(OutPath,ObjFunc_OutName,'Eq_T3', CFM_Stat.Eq_T3,  WriteIT);
     
         write(*,*),'Written Multiple Run Objective Function File'
         write(*,*),'----------------------------------------------------------------------------'
     ...
     END IF !!! TimTick.Run_Cycle_ID

END SUBROUTINE OutPut_MulRun_ObjFunction
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
SUBROUTINE Ctrl_Add_External_Forcing (GrdInfo, Clim_OcnStat, OcnStat, TimTick, SrfFlux, SrfFlux_Hist )
    ...
    IF ( Forcing_DT_Unit == 'C' )THEN
       ...
       IF( (TimTick.CMonth == 1).AND.(TimTick.CDay == 1).AND.(mod(TimTick.Run_AbsIT,Day_NT)==1) ) THEN
           ...
           call NC_Read_Mul_IT(Forcing_Path,SSH_Test_FName,'sshg',&
